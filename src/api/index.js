...
// 请求服务器地址
let API_DOMAIN = 'http://127.0.0.1:6001/'
...
export const apiReqs = {
    Encode: (config) => {
        config.url = API_DOMAIN + 'weid/api/encode'
        config.method = 'post'
        fetch(config)
    },

    TransAct: (config) => {
        config.url = API_DOMAIN + 'weid/api/transact'
        config.method = 'post'
        fetch(config)
    },
    fetchWeId: (config) => {
        config.url = API_DOMAIN + 'weid/api/fetchweid'
        config.method = 'post'
        fetch(config)
    },
    fetchCredential: (config) => {
        config.url = API_DOMAIN + 'weid/api/fetchcredential'
        config.method = 'post'
        fetch(config)
    },
    fetchPresentation: (config) => {
        config.url = API_DOMAIN + 'weid/api/fetchpresentation'
        config.method = 'post'
        fetch(config)
    }
